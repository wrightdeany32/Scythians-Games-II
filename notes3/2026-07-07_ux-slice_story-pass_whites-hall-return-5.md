...
*(Route-neutral base — the certainty sentence ends at "that it meant to," and any run that didn't go deep, including unauthored paths, gets exactly that. The route-specific certainty is appended by one of these `bodyVariants`, per the blessed rule that any terminal referencing a route-specific choice carries a route-neutral base:)*
- **[`return_went_deep`]** append after *"that it meant to"*: **", that it knows you came anyway and didn't turn back"** — the original clause; the run that went all the way down earns the certainty that it was waiting down there.
- **[`return_turned_back`]** append after *"that it meant to"*: **", that it knows you turned back — that you did the careful, sane thing and kept to the shallows, and that it reached past all of it, into a locked truck in an ordinary lot, to put the knife in your hand regardless; that turning around bought you nothing, and depth was never the lock"** — BR-2's run, and crueler for it: the player who did everything right, and was reached anyway. The certainty built on the truck, not the depths.
