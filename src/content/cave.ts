...
You are.`,
    choices: [
      ...
      { label: "Let Reese run ahead.", outcome: { log: "You let Reese run ahead.", tone: "n", setFlags: { cave_lead: "reese" }, queueEvent: "ux_cave_descend" } },
    ],
  },
...
Reese has gone quiet — not spooked, just working, reading the route. Ahead, the passage splits low and mean.`,
    choices: [
      ...
      { label: "Call it — this is getting dangerous.", outcome: { log: "You turn back while it's still easy. A good, boring day.", tone: "g", stats: { grip: 1 }, setFlags: { cave_done: true, cave_turned_back: true } } },
    ],
  },
...
"...tell me you heard that," Reese whispers. And it's strange coming from him, because Reese is the one who explains everything, and right now his voice has a thread in it you've never heard.`,
    choices: [
      { label: `"Probably a bat. They get down this deep."`, outcome: { log: "You give it a name.", tone: "n", queueEvent: "ux_cave_squeeze" } },
      { label: `"That wasn't a bat. That sounded like a voice."`, outcome: { log: "You let it in.", tone: "b", stats: { grip: -1 }, setFlags: { cave_heard_voice: true }, queueEvent: "ux_cave_squeeze" } },
      { label: `"I didn't hear anything. Lamp back on."`, outcome: { log: "You dismiss it.", tone: "n", queueEvent: "ux_cave_squeeze" } },
      { label: `"Please tell me you're not turning into one of those people, Reese."`, outcome: { log: "You rib him, and it grounds you both.", tone: "g", stats: { grip: 1 }, queueEvent: "ux_cave_squeeze" } },
    ],
  },
...
You've done squeezes. You know the math of them — the part where your ribs are the widest thing about you, and the rock does not care.`,
    choices: [
      ...
      { label: `"That's a no. There's the high passage — let's take the long way."`, outcome: { log: "You call the long way.", tone: "n", queueEvent: "ux_cave_otherway" } },
      ...
      { label: `"I'll spot you. Go — I'll follow if it goes."`, outcome: { log: "You send Reese first.", tone: "n", queueEvent: "ux_cave_squeeze_through" } },
    ],
  },
...
"It's teenagers," Reese says, too fast. "It's — this is your imagination doing the thing. Teenagers, they get everywhere." He is already not looking at it.`,
    choices: [
      { label: `"You're right. Teenagers. Little idiots with too much rope."`, outcome: { log: "You choose the version you can live with.", tone: "g", stats: { grip: 1 }, setFlags: { cave_etchings_seen: true }, queueEvent: "ux_cave_return" } },
      { label: `"…I've seen this. Nora had a picture of one just like it."`, outcome: { log: "You let the pattern in — a thread reaches across to Nora.", tone: "b", stats: { grip: -1 }, setFlags: { cave_etchings_seen: true, etchings_link_nora: true }, queueEvent: "ux_cave_return" } },
...
It won't fit with you. You can feel that already. You can wear the pack or wear yourself out fighting it, but not both.`,
    choices: [
      ...
      { label: "Shed the pack — push it ahead, take the loss.", outcome: { log: "You leave gear rather than get stuck.", tone: "n", removeItems: ["cave_gear"], stats: { money: GEAR_LOSS_MONEY }, setFlags: { cave_done: true, cave_deep_seen: true } } },
      // Keep the pack — a tradecraft check. Both branches exit.
      {
        ...
        outcome: {
          roll: {
            tag: "squeeze", statMod: "tradecraft", target: SQUEEZE_TARGET,
            win: { log: "You wear the pack out through the throat, intact.", tone: "g", setFlags: { cave_done: true, cave_deep_seen: true } },
            lose: { log: "You make it — but the pack tears half off on the rock, and something in you doesn't quite come back up.", tone: "b", stats: { grip: -1 }, removeItems: ["cave_gear"], setFlags: { cave_done: true, cave_deep_seen: true } },
          },
        },
      },
