...
- **The two flavor choices are origin seeds, not a branch** — they set no thread flag and resolve identically to the loop. Per the team's confirmation that the creation deck wants them, each carries a distinct origin flag: **"start something" → `origin_fresh_start`; "the one door still open" → `origin_last_door`.** Pure origin seed for the deck; no coordinate, no branch.
