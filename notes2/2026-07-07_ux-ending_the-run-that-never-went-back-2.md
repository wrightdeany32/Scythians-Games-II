...
**[if `doug_gone`]** Doug is somewhere out of state, the family says, looked after, and you send a message once that goes unanswered and you let it be. **[if `doug_lingering`]** Doug still picks you up at six, warm as ever, and you've stopped noticing whether the warmth has an edge, and maybe that's mercy and maybe it's the thing itself, and you'll never do the work of telling them apart. **[if `money_set_aside_doug`]** The pieces you bought that night are in a drawer you don't open — you paid real money for them, money you'd meant for something of your own, and you'll never put them on again, and you can't quite make yourself throw them out either, so they sit, the receipt for a road you walked a little way down and stepped off of. **[if `grave_suspicion` and not `grave_beat_done`]** You never go to the cemetery. Some nights the empty feeling of a grave you've never seen sits with you, and you let it, and you never once drive out to stand in front of it and find out. **[if `grave_confirmed_empty`]** You did stand in front of that vault, once, and you opened it, and you know exactly what isn't in there — and you never told Marie, and you never went back for the rest of it, and you let the knowing sit where it fell, a fact with no home and no proof and nowhere to set it down. **[if `dale_warned_unheeded` or `dale_bond`]** And somewhere at the end of a dead-end road there's a porch light you never took him up on, and an old man who spent fifty years disbelieved and one evening being *heard* — and who will never know that this time, for once, somebody did the thing he begged them to do, and just went home. You gave Dale the only thing he ever asked anyone for, and he will never find out you gave it. **[if `thread_nora_active`]** Nora calls, eventually, from wherever she landed, and you talk about ordinary things, and neither of you says the name of the place, and the not-saying is its own kind of agreement to leave it buried.
...
- **"It was nothing. It was always nothing."** *(you close the door)* → narration only, `attune` leans *grounded* (small). *(narration: "You decide it, and the deciding holds, mostly, the way deciding always mostly holds. There's just the half-second, now and then, at the edge of a dream. You've made your peace with the half-second. It's a small price for a whole life.")* → terminal.
- **"I'll never know."** *(you let the question stay open)* → narration only, `attune` leans *attuned* (small). *(narration: "You don't close it. You carry it, light, the way you'd carry a stone from a beach — a small weight in the pocket, taken out sometimes, turned over, put back. You never look. But you never quite stop wondering, either, and you decide that wondering is a fine thing to keep. Maybe the finest thing you took out of those woods.")* → terminal.
...
- **Thread-echoes gate on flags** — Doug (`doug_gone` / `doug_lingering`); **money, new**: `money_set_aside_doug` — the pieces bought and never worn, a monument to a choice half-made (Courier's pickup); grave, **fixed**: the never-seen echo now gates on `grave_suspicion AND NOT grave_beat_done` (so it never contradicts a player who opened the vault), and a second variant on `grave_confirmed_empty` (the player who *knows* it's empty and still never returned — the heaviest read); **Dale, new**: `dale_warned_unheeded OR dale_bond` — the run where Dale won, the gift he never learns he gave; Nora (`thread_nora_active`). Each is an optional additive paragraph; absent flags drop their echo, so the ending fits any run that reached the calendar's end without returning. (A player who did the grave beat but left it closed, `grave_left_closed`, gets no grave echo here — they had their moment in the grave beat itself.)
- **Two closing reactions carry narration-only `attune`** (grounded/attuned) and nothing else — the player's last *volition* toward the unanswered question, recorded as the derived `attune` scalar (Vigil's option-3 ruling): never a `diamondCoord`, never the centroid or the draw, read only by the ending-selector and telemetry. Changes nothing mechanical here.
