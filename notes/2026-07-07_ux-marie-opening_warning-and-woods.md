...
- **Loop-native, frozen-cave-safe.** Met-door (`scheduleEvent`, gated `arrived_town`) → a scheduled outing → a queue-chained conversation. Reads `arrived_town`, `cave_done`, `doug_off`, `mausoleum_known`; writes `marie_*`, `knows_ellen`, `grave_suspicion`, `pointed_to_dale`, `pattern_open`.
